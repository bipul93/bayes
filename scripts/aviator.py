...
def motion_model(rot1, trans, rot2):
    ...
    for cell_index in range(cell[0].shape[0]):
        ...
        for ind, data in numpy.ndenumerate(grid):
            ...
            # p1 = norm(0, (math.pi / 8)).pdf(rot1[2] - rot1_cap)
            # p2 = norm(0, 10).pdf(trans - trans_cap)
            # p3 = norm(0, (math.pi / 8)).pdf(rot2[2] - rot2_cap)
    ...
    for i in numpy.arange(grid.shape[0]):
        for j in numpy.arange(grid.shape[1]):
            for k in numpy.arange(grid.shape[2]):
                ...
                k2 = normalize(k * (math.pi / 6) + (math.pi / 12))

                trans_cap = numpy.sqrt((x1 - x2) ** 2 + (y1 - y2) ** 2)
                rot1_cap = normalize(math.atan2(y1 - y2, x1 - x2) - k2)
                # rot2_cap = k2 - rot1_cap

                # p1 = norm(0, (math.pi / 8)).pdf(bearing[2] - rot1_cap)
                # p2 = norm(0, 10).pdf(tag_range * 100 - trans_cap)
